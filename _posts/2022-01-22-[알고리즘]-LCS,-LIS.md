...
length(k)의 값을 계산하기 위해서는 array[i]< array[k]이면서 length(i)가 최대가 되는 위치 i < k를 찾아야 한다. 이는 i에서 끝나는 최장 증가 부분 수열의 마지막 array[k]를 추가하는 것이 최적이기 때문에 length(k) = length(i) +1이다. 만족하는 i가 없다면 부분 수열은 array[k]만을 포함하게 되므로 length[k] = 1이 된다. 
