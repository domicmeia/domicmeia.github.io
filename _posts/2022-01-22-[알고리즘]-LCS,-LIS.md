...
## DP 접근 방법
Brute-Force 접근 방법은 입력이 배열로 주어져 메모이제이션을 적용하기 어렵다. 이를 보완한게 DP이다.  
length(k)를 위치 k에서 끝나는 최장 증가 부분 수열의 길이라고 했을 때, 0 <= k <= n-1를 만족하는 모든 k에 대해 length(k)를 계산하여 최장 증가 부분 수열의 길이를 구할 수 있다. 예를 들어 **S = {6, 2, 5, 1, 7, 4, 8, 3}**이라 하자 그러면 함수의 값은 다음과 같다.  
...
length(k)의 값을 계산하기 위해서는 array[i]< array[k]이면서 length(i)가 최대가 되는 위치 i < k를 찾아야 한다. 이는 i에서 끝나는 최장 증가 부분 수열의 마지막 array[k]를 추가하는 것이 최적이기 때문에 **length(k) = length(i) +1**이다. 만족하는 i가 없다면 부분 수열은 array[k]만을 포함하게 되므로 **length[k] = 1**이 된다. 
