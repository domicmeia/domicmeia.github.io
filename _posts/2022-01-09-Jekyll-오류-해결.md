...
Jekyll에서 사용되는 liquid가 `{%raw%}{{{%endraw%}` 와 `{%raw%}}}{%endraw%}`를 escape 문자로 사용하기 때문에 발생하는 오류이다. 코드 안에 이중 중괄호가 있으면 jekyll engine이 경고 메세지를 출력하고, {%raw%}{{{%endraw%} ... {%raw%}}}{%endraw%} 사이에 있는 내용은 무시된다.
